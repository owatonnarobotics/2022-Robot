#pragma once

const int R_ShooterCANID = 13;
const int R_IndexerCANID = 14;
const int R_IntakeCANID = 15;
const int R_ClimberCANID = 16;
const int R_ClimberCANID2 = 17;
const int R_LimitSwitch = 0;
const int R_SpinnerCANID = 20;
const double R_secondShooterSpeed = .92;
const int R_ClimberSolenoid = 0;
const int R_LimitSwitch2 = 1;
const double R_shooterVelocityClose = 240;
const double R_spinnerVelocityClose = 300;
const double R_shooterVelocityFar = 220;
const double R_spinnerVelocityFar = 430;

